...
void keyboard(unsigned char key, int x, int y)
{
	...
	case '1':
		...
		cout << l.light_position[0] << "-" << l.light_position[1] << "-" << l.light_position[2] << endl;
		break;
...
bool is_front(Vector3D obj_vec[3])
{
	...
	light_vector.x = obj_vec[0].x - l.light_position[0];
	light_vector.y = obj_vec[0].y - l.light_position[1];
	light_vector.z = obj_vec[0].z - l.light_position[2];

	return (cross_result.Dot(light_vector) >= 0) ? false : true;
}
